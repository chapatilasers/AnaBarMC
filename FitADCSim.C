...
static const Float_t pedastel_sigma = 2.79;
...
void FitADCSim(Int_t Analysis_Run_Number = 88811, Int_t runno = 1550, Int_t n_events=-1) {
...
TCanvas *plotC9 (/*Float_t Theta_min_cut = 3.017*/ Float_t Theta_min_cut = 0.0, Float_t Edep_Threshold = 0.0, Int_t Nphot_Neighbor_Cut = 10, Int_t Analyse_Secondaries = 1){
  ...
  TF1 *fcn;
  Double_t means[NUMPADDLE], sigmas[NUMPADDLE], meanErr[NUMPADDLE], sigErr[NUMPADDLE];
  ...
  for (Int_t i = 0; i < NUMPADDLE; i++){
	...
	Double_t bc =0.0;
	Double_t bn = 0.0;
	Int_t nbin = AnaBar_NPhotons_Max*0.8+10;
	Int_t min = -10;
...
TCanvas *plot_adc_fit(Int_t pmt=1, Int_t tdc_min=850, Int_t tdc_width=100, Int_t adc_neighbor_cut=10, Int_t adc_cut=50, Int_t max=1000){
	...
	Double_t meanPE[NUMPIXEL], sigmaPE[NUMPIXEL];
	...
	for(Int_t i = 0; i < NUMPIXEL; i++) {
		...
		PEperADCs[i] = PEperMeV*7.12/means[i];
		//minPEs[i] = 0;
	...
	cout << "Suggested Calibration = " << PEperMeV*7.12/average_mean << endl;
	cout << "Suggested Smearing = " << average_sigma*PEperMeV*7.12/average_mean << endl;
