...
void effplot_ejb(){
  ...
  TH1D *hh = new TH1D[NUMBINS];   // adc spectrum with TDC & ADC neighbors cut
  TH1D *ha = new TH1D[NUMBINS];   // adc with TDC crosstalk cut added to previous cuts
  TH1D *hb = new TH1D[NUMBINS];   // adc with TDC width cut included also
  TH1D *hc = new TH1D[NUMBINS];   // adc with threshold cut added to other cuts

  TCut temph1,temph2,temph3;
  TCut* hcuts = new TCut[NUMPADDLE];
  TCut* acuts = new TCut[NUMPADDLE];
  TCut* bcuts = new TCut[NUMBINS];
  TCut* ccuts = new TCut[NUMBINS];
   ...
   for(Int_t id=0; id<100;id++){
    ...
    for(Int_t canvas=0; canvas < NUMPIXEL; canvas++){
     if(canvas!=pixel1[pmt-1]-1 && canvas!=pixel2[pmt-1]-1){
        ...
        //t[hvolt-1]->SetLineColor(1);
	temptitle1.Form("C.cdetm1r.adc_c[%d] >> hh[%d]",paddleindex[paddle],canindex);
	if(t[hvolt-1]->Draw(temptitle1, hcuts[canindex],"goff",1,id)) {
	       	//cout << hcuts[canindex].GetTitle() << endl;
		cout << "paddleindex = " << paddleindex[paddle] << " canindex = " << canindex << " hcuts passed" << endl;
	}
	//t[hvolt-1]->SetLineColor(4);
	temptitle2.Form("C.cdetm1r.adc_c[%d] >> ha[%d]",paddleindex[paddle],canindex);
	if(t[hvolt-1]->Draw(temptitle2, acuts[canindex],"goff",1,id)) {
		//cout << acuts[canindex].GetTitle() << endl;
		cout << "paddleindex = " << paddleindex[paddle] << " canindex = " << canindex << " acuts passed" << endl;
	}
	//t[hvolt-1]->Draw(temptitle2, acuts[canindex], "same");
	
	//t[hvolt-1]->SetLineColor(2);
	temptitle3.Form("C.cdetm1r.adc_c[%d] >> hb[%d]",paddleindex[paddle],canindex);
	if(t[hvolt-1]->Draw(temptitle3, bcuts[canindex+(hvolt-1)*NUMPADDLE],"goff",1,id)) cout << "paddleindex = " << paddleindex[paddle] << " canindex = " << canindex << " bcuts passed" << endl;
	//t[hvolt-1]->Draw(temptitle3, bcuts[canindex+(hvolt-1)*NUMPADDLE], "same");
	
	//t[hvolt-1]->SetLineColor(3);
	//t[hvolt-1]->SetFillColor(3);
	temptitle4.Form("C.cdetm1r.adc_c[%d] >> hc[%d]",paddleindex[paddle],canindex);
	if(t[hvolt-1]->Draw(temptitle4, ccuts[canindex+(hvolt-1)*NUMPADDLE],"goff",1,id)) cout << "paddleindex = " << paddleindex[paddle] << " canindex = " << canindex << " ccuts passed" << endl;
	//t[hvolt-1]->Draw(temptitle4, ccuts[canindex+(hvolt-1)*NUMPADDLE], "same");
	//t[hvolt-1]->SetFillColor(0);
	
	
	//cout << "bf calculation: " << canindex+(hvolt-1)*NUMPADDLE << endl;
	//bf[canindex+(hvolt-1)*NUMPADDLE] = t[hvolt-1]->GetEntries(bcuts[canindex+(hvolt-1)*NUMPADDLE].GetTitle());
	//cout << "af calculation: " << canindex+(hvolt-1)*NUMPADDLE << endl;
	//af[canindex+(hvolt-1)*NUMPADDLE] = t[hvolt-1]->GetEntries(ccuts[canindex+(hvolt-1)*NUMPADDLE].GetTitle());
	//cout << "eff calculation: " << af[canindex+(hvolt-1)*NUMPADDLE] << " " << bf[canindex+(hvolt-1)*NUMPADDLE] << endl;
	//eff[canindex+(hvolt-1)*NUMPADDLE] = (af[canindex+(hvolt-1)*NUMPADDLE])/(bf[canindex+(hvolt-1)*NUMPADDLE]);
